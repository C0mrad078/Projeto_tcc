...
agent/
  ...
  adapters.py              estratégias de identidade plugáveis (per_module/shared — generalização)
...
- **Componentes ainda específicos do vAPI**: `setup_vapi_users.py` inteiro (registro de usuários é sempre específico de cada alvo — não há como generalizar sem reescrever para o alvo novo). A resolução de identidade em si (`Config.identity()`) já é plugável desde a Fase 8 (`IDENTITY_STRATEGY=per_module|shared` — ver `agent/adapters.py`), mas só a estratégia "per_module" (a do vAPI) foi validada contra um alvo real; "shared" existe e tem teste unitário, mas nunca rodou contra um alvo de verdade. O restante do pipeline (`openapi_parser`, `relation_inference`, `test_generator`, `http_client`, `executor`, `classifier`) é genérico.
